...
const SwitchThemed = styled(Switch)(({ theme }) => ({
  ...
  '& .MuiSwitch-thumb': {
    ...
    '&:before': {
      ...
      backgroundImage:
        theme.palette.mode === 'dark'
          ? `url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" height="20" width="20" viewBox="0 0 20 20"><path fill="${encodeURIComponent(
              '#fff',
...
export default function StyledSwitch(): ReactNode {
  ...
  return (
    <Tooltip title={themeState.dark ? 'Switch to Light Mode' : 'Switch to Dark Mode'}>
      <SwitchThemed
        checked={currentTheme.includes('dark') || themeState.dark}
        onClick={switchTheme}
      />
    </Tooltip>
  );
