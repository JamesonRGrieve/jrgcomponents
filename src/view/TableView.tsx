                ...
                <TableBody>
                  {
                    props.rows.map((row: any) =>
                      <TableRow key={row.name}>
                        ...
                        {
                          row.linkAccessor?
                          <TableCell align="center"><Link sx={{cursor: "pointer"}} onClick={() => Router.push(row.linkAccessor(props.data))}>{row.valueAccessor(props.data) ?? "Not Available"}</Link></TableCell>
                          :
